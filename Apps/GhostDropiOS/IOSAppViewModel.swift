...
final class IOSAppViewModel: ObservableObject {
    ...
    private let logger = GhostLogger.shared
    ...
    private var selectedPeer: NearbyDevice?
    ...
    func sendSelectedFile() {
        ...
        Task {
            do {
                try await senderSession?.sendFile(at: fileURL, mimeType: "application/octet-stream")
            } catch {
                errorMessage = error.localizedDescription
            }
        }
    ...
    func exportLogs() {
        Task {
            do {
                let destination = FileManager.default.temporaryDirectory.appendingPathComponent("ghostdrop-ios-log.ndjson")
                try await senderSession?.exportLogsNDJSON(to: destination)
                try await receiverSession?.exportLogsNDJSON(to: destination)
                errorMessage = "Logs exported: \(destination.lastPathComponent)"
            } catch {
                errorMessage = error.localizedDescription
            }
        }
